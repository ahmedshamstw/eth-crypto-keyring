...
const BRIDGE_URL = 'https://metamask.github.io/eth-crypto-bridge-keyring'
...
const CONNECTION_EVENT = 'crypto-connection-change'
...
class CryptoBridgeKeyring extends EventEmitter {
        ...
        .then(async (_) => {
          ...
          for (let i = from; i < to; i++) {
            ...
            this.accountDetails[ethUtil.toChecksumAddress(address)] = {
              ...
              // It's currently used to represent whether an account uses the Crypto Live path.
  ...
  updateTransportMethod (transportType) {
    return new Promise((resolve, reject) => {
      ...
      this._sendMessage({
        action: 'crypto-update-transport',
        params: { transportType },
      }, ({ success }) => {
        if (success) {
          resolve(true)
        } else {
          reject(new Error('Crypto transport could not be updated'))
        }
      })
    })
  }
  ...
  _signTransaction (address, rawTxHex, handleSigning) {
    return new Promise((resolve, reject) => {
      this.unlockAccountByAddress(address)
        .then((hdPath) => {
          this._sendMessage({
            action: 'crypto-sign-transaction',
            params: {
              tx: rawTxHex,
              hdPath,
            },
          ...
          ({ success, payload }) => {
            if (success) {
              ...
              if (valid) {
                ...
                reject(new Error('Crypto: The transaction signature is not valid'))
              }
            } else {
              reject(payload.error || new Error('Crypto: Unknown error while signing transaction'))
            }
          })
        })
        .catch(reject)
  ...
  signPersonalMessage (withAccount, message) {
    return new Promise((resolve, reject) => {
      this.unlockAccountByAddress(withAccount)
        .then((hdPath) => {
          this._sendMessage({
            action: 'crypto-sign-personal-message',
            params: {
              hdPath,
          ...
          ({ success, payload }) => {
            if (success) {
              ...
              if (ethUtil.toChecksumAddress(addressSignedWith) !== ethUtil.toChecksumAddress(withAccount)) {
                reject(new Error('Crypto: The signature doesnt match the right address'))
              }
              resolve(signature)
            } else {
              reject(payload.error || new Error('Crypto: Unknown error while signing message'))
            }
          })
        })
        .catch(reject)
  ...
  async unlockAccountByAddress (address) {
    ...
    if (!Object.keys(this.accountDetails).includes(checksummedAddress)) {
      throw new Error(`Crypto: Account for address '${checksummedAddress}' not found`)
    }
    const { hdPath } = this.accountDetails[checksummedAddress]
    const unlockedAddress = await this.unlock(hdPath, false)

    // unlock resolves to the address for the given hdPath as reported by the crypto device
    // if that address is not the requested address, then this account belongs to a different device or seed
    if (unlockedAddress.toLowerCase() !== address.toLowerCase()) {
      throw new Error(`Crypto: Account ${address} does not belong to the connected device`)
    }
    return hdPath
  }
  ...
  async signTypedData (withAccount, data, options = {}) {
    ...
    if (!isV4) {
      throw new Error('Crypto: Only version 4 of typed data signing is supported')
    }
    ...
    if (success) {
      ...
      if (ethUtil.toChecksumAddress(addressSignedWith) !== ethUtil.toChecksumAddress(withAccount)) {
        throw new Error('Crypto: The signature doesnt match the right address')
      }
      return signature
    }
    throw payload.error || new Error('Crypto: Unknown error while signing message')
  }
  ...
  _setupIframe () {
    ...
    this.iframe.onload = async () => {
      // If the crypto live preference was set before the iframe is loaded,
  ...
  _getPathForIndex (index) {
    // Check if the path is BIP 44 (Crypto Live)
